...
TRAIN_MANIFEST="${TRAIN_MANIFEST:-brain_text_pipeline/data/meg_aligned_postword_subject_train/manifest.json}"
TEST_MANIFEST="${TEST_MANIFEST:-brain_text_pipeline/data/meg_aligned_postword_subject_test/manifest.json}"
RUN_ROOT="${RUN_ROOT:-brain_text_pipeline/runs}"
PRIMARY_PREFIX="${PRIMARY_PREFIX:-t5_meg_postword_subject_hybrid_last1}"
PRIMARY_NORM="${PRIMARY_NORM:-per_example}"
SEEDS="${SEEDS:-42 7 123}"
RUN_ALT_NORM="${RUN_ALT_NORM:-0}"
...
train_and_eval() {
  ...
  local brain_norm="$2"
  local run_dir="$3"
  local eval_json="${run_dir}/eval_subject_test_50k.json"
...
for seed in ${SEEDS}; do
  ...
  eval_jsons+=("${run_dir}/eval_subject_test_50k.json")
done
...
"${PYTHON_BIN}" brain_text_pipeline/scripts/summarize_meg_subject_multiseed.py \
