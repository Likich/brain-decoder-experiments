...
\begin{figure*}[t]
    \centering
    \includegraphics[width=\linewidth]{fig_overview_paired_control.png}
    \caption{
    Overview of the paper.
    We study brain-conditioned language modeling in two regimes: synthetic TVB states $z\in\mathbb{R}^d$ for controlled mechanism studies, and real post-word MEG windows $B_{1:T}\in\mathbb{R}^{T\times C}$ for held-out validation.
    Both are encoded and used to condition a pretrained T5 via cross-attention.
    The core evaluation is paired-control: REAL uses the matched brain input, ZERO removes it, and SHUF replaces it with a mismatched input from another example.
    Brain conditioning is considered useful only when REAL improves predictive utility over both ZERO and SHUF, while JSD and top-1 change quantify the size of the induced distributional deformation.
    }
    \label{fig:pipeline}
\end{figure*}
...
(iii) On real MEG-MASC recordings, we show that the same criterion survives held-out evaluation: a T5 cross-attention model with post-word MEG windows improves target-word NLL over both ZERO and SHUF on random and story-blocked splits after correcting decoder leakage and signal-scale failures.
...
We report both random train/test splits and story-blocked held-out evaluation to reduce story-level leakage.
...
We therefore evaluate the stronger post-word hybrid configuration on two held-out regimes: a standard random split and a story-blocked split that withholds entire stories at test time.
...
After removing decoder-context leakage and fixing MEG scaling, the post-word T5 model improves target-word NLL over both ZERO and SHUF on random and story-blocked splits, with $\Delta$NLL gains of roughly $0.08$--$0.11$.
...
The MEG-MASC pilot narrows that gap, but its effect remains token-level and modest, and we have not yet tested subject-blocked or leave-one-subject-out generalization.
...
On real MEG-MASC, the same REAL/ZERO/SHUF logic survives random and story-blocked held-out evaluation after correcting leakage and signal-scaling failures.
...
Both random held-out and story-blocked evaluation preserve the REAL $<$ ZERO and REAL $<$ SHUF control pattern.
}
...
Random held-out and story-blocked evaluation both preserve a clear REAL advantage over ZERO and SHUF, while the peri-word variant remains a cautionary example that stronger brain dependence can still be harmful.
